      ...
      if (semanticIntegration.canUseSemanticChunking(fileInfos)) {
        ...
        if (!semanticResult.fallbackUsed && semanticResult.chunks.length > 0) {
          ...
          if (semanticResult.method === 'semantic') {
            ...
            if (hasConsolidation) {
              const totalOriginalThreads = semanticResult.chunks.reduce((sum: number, chunk) => 
            ...
            logger.info(`   • Files analyzed: ${semanticResult.chunks.reduce((sum: number, chunk) => sum + ((chunk as any).files?.length || 0), 0)}`);
            // Show traditional chunk details
            semanticResult.chunks.forEach((chunk, index: number) => {
              logger.info(`   • Chunk ${index + 1}: ${(chunk as any).files?.length || 0} files, ~${chunk.estimatedTokens} tokens`);
            });
          }
