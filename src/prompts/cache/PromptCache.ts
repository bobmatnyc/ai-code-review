...
export class PromptCache {
  ...
  static getInstance(cacheDir?: string): PromptCache {
    if (!PromptCache.instance) {
      ...
      PromptCache.instance.initialize().catch(error => {
        logger.error(`Error initializing prompt cache: ${error instanceof Error ? error.message : String(error)}`);
      });
    }
      ...
      for (const file of files) {
        if (file.endsWith('.json')) {
          try {
            ...
            logger.error(`Error loading cache file ${file}: ${error instanceof Error ? error.message : String(error)}`);
          }
        }
      }
    } catch (error) {
      logger.error(`Error loading cached prompts: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private async saveCacheToDisk(): Promise<void> {
    try {
      ...
      logger.error(`Error saving prompt cache: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  getBestPrompt(reviewType: ReviewType): CachedPrompt | undefined {
    try {
      ...
      logger.error(`Error getting best prompt: ${error instanceof Error ? error.message : String(error)}`);
      return undefined;
    }
  }
  ...
  getAllPrompts(reviewType: ReviewType): CachedPrompt[] {
    try {
      ...
      logger.error(`Error getting all prompts: ${error instanceof Error ? error.message : String(error)}`);
      return [];
    }
  }
  ...
  async clearAllCaches(): Promise<void> {
    try {
      ...
      logger.error(`Error clearing all caches: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
