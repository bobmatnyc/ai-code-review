...
export const ImprovedUnusedCodeIssueSchema = z.object({
  ...
  assessment: z.object({
    ...
    staticAnalysisHint: z.string().optional().describe('Hint for static analysis tool configuration that could catch this')
  }),
  ...
  suggestedAction: z.object({
    ...
    replacement: z.string().optional().describe('Suggested replacement code if refactoring is recommended'),
   ...
   */
  isCompleteElement: z.boolean().describe('True if this is a complete element like a file, function, or class that can be removed entirely'),
  
  /**
   * Potential dependencies or references that should be checked
   */
  relatedChecks: z.array(z.string()).optional().describe('Additional files or components that should be checked to confirm this is truly unused')
});
...
export const ImprovedUnusedCodeReviewSchema = z.object({
   ...
   */
  highImpactIssues: z.array(ImprovedUnusedCodeIssueSchema)
    .describe('High impact unused code issues'),
  
  /**
   * Array of medium impact unused code issues
   */
  mediumImpactIssues: z.array(ImprovedUnusedCodeIssueSchema)
    .describe('Medium impact unused code issues'),
  
  /**
   * Array of low impact unused code issues
   */
  lowImpactIssues: z.array(ImprovedUnusedCodeIssueSchema)
   ...
   */
  codebasePatterns: z.array(z.object({
