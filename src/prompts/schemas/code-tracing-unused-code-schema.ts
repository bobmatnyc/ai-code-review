...
export const TraceEvidenceSchema = z.object({
  ...
  ).optional().describe('Export information if the element is exported'),
   ...
   */
  edgeCasesConsidered: z.array(z.object({
    /**
   ...
   * Additional evidence
   */
  additionalEvidence: z.string().optional().describe('Additional evidence supporting this conclusion')
});
...
export const TracedUnusedElementSchema = z.object({
   ...
   */
  confidence: z.enum(['high', 'medium', 'low']).describe('Confidence level that this element is truly unused'),
...
export const CodeTracingUnusedCodeReviewSchema = z.object({
   ...
   * Unused files
   */
  unusedFiles: z.array(TracedUnusedElementSchema.refine(val => val.elementType === 'file', {
   ...
   * Unused functions
   */
  unusedFunctions: z.array(TracedUnusedElementSchema.refine(val => ['function', 'hook'].includes(val.elementType as string), {
   ...
   * Unused classes
   */
  unusedClasses: z.array(TracedUnusedElementSchema.refine(val => ['class', 'component'].includes(val.elementType as string), {
   ...
   */
  unusedTypesAndInterfaces: z.array(TracedUnusedElementSchema.refine(val => ['interface', 'type', 'enum'].includes(val.elementType as string), {
   ...
   * Dead code branches
   */
  deadCodeBranches: z.array(TracedUnusedElementSchema.refine(val => val.elementType === 'dead-branch', {
   ...
   */
  unusedVariablesAndImports: z.array(TracedUnusedElementSchema.refine(val => ['variable', 'import', 'parameter', 'property', 'export'].includes(val.elementType as string), {
  ...
  summary: z.object({
    ...
    potentialCodeReduction: z.string().describe('Estimated percentage of code that could be removed')
