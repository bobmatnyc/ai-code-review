...
export const UnusedElementSchema = z.object({
   ...
   */
  dependents: z.array(z.string()).optional().describe('Elements that depend on this element (if any)'),
...
export const FocusedUnusedCodeReviewSchema = z.object({
   ...
   * Unused files
   */
  unusedFiles: z.array(UnusedElementSchema.refine(val => val.elementType === 'file', {
   ...
   * Unused functions
   */
  unusedFunctions: z.array(UnusedElementSchema.refine(val => ['function', 'hook'].includes(val.elementType as string), {
   ...
   * Unused classes
   */
  unusedClasses: z.array(UnusedElementSchema.refine(val => ['class', 'component'].includes(val.elementType as string), {
   ...
   */
  unusedTypesAndInterfaces: z.array(UnusedElementSchema.refine(val => ['interface', 'type', 'enum'].includes(val.elementType as string), {
   ...
   * Dead code branches
   */
  deadCodeBranches: z.array(UnusedElementSchema.refine(val => val.elementType === 'dead-branch', {
   ...
   */
  unusedVariablesAndImports: z.array(UnusedElementSchema.refine(val => ['variable', 'import', 'parameter', 'property', 'export'].includes(val.elementType as string), {
