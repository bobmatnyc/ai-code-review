...
import { ImprovedUnusedCodeReview, ImprovedUnusedCodeIssue, UnusedCodeCategory } from '../prompts/schemas/improved-unused-code-schema';
...
export function formatUnusedCodeReviewAsMarkdown(review: ImprovedUnusedCodeReview): string {
  ...
  if (unusedFiles.length > 0) {
    ...
    const highConfidenceFiles = unusedFiles.filter(issue => issue.assessment.confidence === 'high');
    const mediumConfidenceFiles = unusedFiles.filter(issue => issue.assessment.confidence === 'medium');
  ...
  if (unusedFunctions.length > 0) {
    ...
    const highConfidenceFunctions = unusedFunctions.filter(issue => issue.assessment.confidence === 'high');
    const mediumConfidenceFunctions = unusedFunctions.filter(issue => issue.assessment.confidence === 'medium');
    const lowConfidenceFunctions = unusedFunctions.filter(issue => issue.assessment.confidence === 'low');
  ...
  if (unusedClasses.length > 0) {
    ...
    const highConfidenceClasses = unusedClasses.filter(issue => issue.assessment.confidence === 'high');
    const mediumConfidenceClasses = unusedClasses.filter(issue => issue.assessment.confidence === 'medium');
    const lowConfidenceClasses = unusedClasses.filter(issue => issue.assessment.confidence === 'low');
  ...
  if (unusedModules.length > 0) {
    ...
    const highConfidenceModules = unusedModules.filter(issue => issue.assessment.confidence === 'high');
    const mediumConfidenceModules = unusedModules.filter(issue => issue.assessment.confidence === 'medium');
    const lowConfidenceModules = unusedModules.filter(issue => issue.assessment.confidence === 'low');
  ...
  if (otherUnusedElements.length > 0) {
    ...
    const highConfidenceOther = otherUnusedElements.filter(issue => issue.assessment.confidence === 'high');
    const mediumConfidenceOther = otherUnusedElements.filter(issue => issue.assessment.confidence === 'medium');
    const lowConfidenceOther = otherUnusedElements.filter(issue => issue.assessment.confidence === 'low');
...
function getAllIssuesByCategory(
  ...
  excludeMainCategories: boolean = false
...
function formatIssuesAsChecklist(issues: ImprovedUnusedCodeIssue[]): string {
  ...
  for (const [filePath, fileIssues] of Object.entries(issuesByFile)) {
    ...
    for (const issue of fileIssues) {
      const lines = issue.location.lineStart && issue.location.lineEnd 
...
export function formatUnusedCodeReviewForTerminal(review: ImprovedUnusedCodeReview): string {
  // Build the header
  let output = chalk.bold.blue('UNUSED CODE REVIEW: FILES & FUNCTIONS THAT CAN BE SAFELY REMOVED\n\n');
  ...
  if (unusedFiles.length > 0) {
    ...
    output += chalk.italic('The following files are not imported or used anywhere and can be safely removed:\n\n');
  ...
  if (unusedFunctions.length > 0) {
    ...
    output += chalk.italic('The following functions are never called and can be safely removed:\n\n');
 ...
 * @returns Formatted string for terminal output
 */
function formatIssuesForTerminal(issues: ImprovedUnusedCodeIssue[], color: 'green' | 'yellow' | 'red'): string {
  let output = '';
...
export function generateRemovalScript(review: ImprovedUnusedCodeReview): string {
  ...
  const unusedFunctions = getAllIssuesByCategory(review, 'unusedFunction')
    .filter(issue => issue.assessment.confidence === 'high' && issue.isCompleteElement);
  ...
  const otherHighConfidenceIssues = getAllIssuesByCategory(review, null, true)
    .filter(issue => issue.assessment.confidence === 'high');
