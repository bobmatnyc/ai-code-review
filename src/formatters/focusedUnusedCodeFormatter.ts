...
import { FocusedUnusedCodeReview, UnusedElement } from '../prompts/schemas/focused-unused-code-schema';
...
export function formatFocusedUnusedCodeReviewForTerminal(review: FocusedUnusedCodeReview): string {
  ...
  if (review.unusedFiles.length > 0) {
    ...
    output += chalk.italic('The following files are never imported or used and can be safely removed:\n\n');
 ...
 * @returns Formatted string for terminal output
 */
function formatElementsForTerminal(elements: UnusedElement[], color: 'green' | 'yellow' | 'red'): string {
  let output = '';
