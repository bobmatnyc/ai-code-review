...
import { CodeTracingUnusedCodeReview, TracedUnusedElement } from '../prompts/schemas/code-tracing-unused-code-schema';
 ...
 */
export function formatCodeTracingUnusedCodeReviewAsMarkdown(review: CodeTracingUnusedCodeReview): string {
  // Build the header
  ...
  if (review.unusedTypesAndInterfaces.length > 0) {
    ...
    // Group by confidence
    const highConfidence = review.unusedTypesAndInterfaces.filter(type => type.confidence === 'high');
    const mediumConfidence = review.unusedTypesAndInterfaces.filter(type => type.confidence === 'medium');
    const lowConfidence = review.unusedTypesAndInterfaces.filter(type => type.confidence === 'low');
  ...
  if (review.deadCodeBranches.length > 0) {
    ...
    const mediumConfidence = review.deadCodeBranches.filter(branch => branch.confidence === 'medium');
  ...
  if (review.unusedVariablesAndImports.length > 0) {
    ...
    // Group by confidence
    const highConfidence = review.unusedVariablesAndImports.filter(variable => variable.confidence === 'high');
    const mediumConfidence = review.unusedVariablesAndImports.filter(variable => variable.confidence === 'medium');
    const lowConfidence = review.unusedVariablesAndImports.filter(variable => variable.confidence === 'low');
  ...
  for (const [filePath, fileElements] of Object.entries(elementsByFile)) {
    ...
    for (const element of fileElements) {
      ...
      if (element.location.startLine && element.location.startLine > 0) {
        location = element.location.endLine && element.location.endLine > 0
      ...
      const defLine = element.evidence.definition.line && element.evidence.definition.line > 0 
      ...
      if (element.evidence.exports && element.evidence.exports.length > 0) {
        ...
        for (const exportInfo of element.evidence.exports) {
          ...
          const exportLine = exportInfo.line && exportInfo.line > 0 
            ? `:${exportInfo.line}` 
            : '';
