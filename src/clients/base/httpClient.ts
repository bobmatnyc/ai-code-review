  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (res.status === 429 || res.status >= 500) {
        ...
        const delayMs = retryAfter ? parseInt(retryAfter, 10) * 1000 : 1000 * Math.pow(2, i);
      ...
      if (i < retries - 1 && !(error instanceof ApiError)) {
        ...
        logger.warn(`Request failed with error: ${error instanceof Error ? error.message : String(error)}. Retrying in ${delayMs / 1000}s...`);
        await new Promise(r => setTimeout(r, delayMs));
      } else {
        throw error;
      }
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (
        ...
        const delayMs = 1000 * Math.pow(2, i);
        logger.warn(`Operation failed with status ${err.status}. Retrying in ${delayMs / 1000}s...`);
        await new Promise(r => setTimeout(r, delayMs));
      } else if (i < retries - 1) {
        // For other errors, retry with backoff
        const delayMs = 1000 * Math.pow(2, i);
        logger.warn(`Operation failed: ${error instanceof Error ? error.message : String(error)}. Retrying in ${delayMs / 1000}s...`);
        await new Promise(r => setTimeout(r, delayMs));
      } else {
        throw error;
      }
