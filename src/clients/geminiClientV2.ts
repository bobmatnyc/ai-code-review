...
export class GeminiClient extends BaseAiClient {
  ...
  public async generateReview(
    ...
    try {
      ...
      const promptTemplate = await loadPromptTemplate(
        options.reviewType || 'quick-fixes',
        fileInfo.language
      );
      ...
      const codeBlock = this.formatCodeBlock(fileInfo.content, fileInfo.language);
      
      // Build the prompt
      const prompt = promptTemplate
        .replace('{{FILE_PATH}}', fileInfo.filePath)
      ...
      if (streamHandler && this.model?.generateContentStream) {
        ...
        for await (const chunk of result.stream) {
          ...
          if (streamHandler.onToken) {
            streamHandler.onToken(chunkText);
          }
        }
      ...
      return {
        content: `Error generating review: ${error.message}`,
        model: this.currentModel,
        error: error
      };
    }
  ...
  public async generateConsolidatedReview(
    ...
    try {
      ...
      const fileSummaries = files.map((file) => {
        return `File: ${file.filePath}\nLanguage: ${file.language || 'unknown'}\n\n${this.formatCodeBlock(file.content, file.language)}\n\n`;
      }).join('\n---\n\n');
      ...
      return {
        content: `Error generating consolidated review: ${error.message}`,
        model: this.currentModel,
        error: error
      };
    }
  ...
  private async generateMockReview(
    ...
    const mockResponse = mockTemplate
      .replace('{{FILE_PATH}}', fileInfo.filePath)
      .replace('{{REVIEW_TYPE}}', options.reviewType || 'quick-fixes')
