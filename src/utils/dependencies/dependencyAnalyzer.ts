...
export async function analyzeDependencies(directory: string): Promise<DependencyAnalysisResult> {
  ...
  try {
    ...
    logger.error(`Error analyzing dependencies: ${error instanceof Error ? error.message : String(error)}`);
    return {
      modulesCount: 0,
      dependenciesCount: 0,
 ...
 */
function processAnalysisResult(data: DependencyCruiserOutput, graphPath: string): DependencyAnalysisResult {
  ...
  const topViolations = data.summary.violations
    .slice(0, 10)
    .map(violation => ({
      type: violation.type,
      from: violation.from,
