 ...
 */
export async function analyzePackageSecurity(projectPath: string): Promise<PackageSecurityAnalysisResult> {
  logger.info('Starting package security analysis...');
  
  // Check if SERPAPI is configured
  if (!hasSerpApiConfig()) {
    logger.warn('SERPAPI_KEY not found in environment variables. Security analysis requires this key to be set.');
    return {
      analyzedPackages: [],
      packageCount: 0,
  ...
  try {
    ...
    logger.error(`Error analyzing package security: ${error instanceof Error ? error.message : String(error)}`);
    return {
      analyzedPackages: [],
      packageCount: 0,
      securityIssuesFound: 0,
...
async function analyzePackageFile(packageResult: PackageAnalysisResult): Promise<{
  ...
  if (securityResults.length === 0) {
    ...
    for (const result of securityResults) {
      ...
      const significantVulnerabilities = result.vulnerabilities.filter(
