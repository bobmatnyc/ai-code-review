...
export async function analyzePackagesWithStackAwareness(
  ...
  try {
    ...
    if (primaryStack) {
      logger.info(`Primary tech stack detected: ${primaryStack.name} (${primaryStack.confidence} confidence)`);
    } else {
    ...
    logger.error(`Error analyzing packages with stack awareness: ${error instanceof Error ? error.message : String(error)}`);
    return {
      detectedStacks: [],
      packageResults: [],
      allPackages: [],
...
function isFrameworkPackage(packageName: string, stackType: TechStackType): boolean {
  switch (stackType) {
    case 'react':
      return packageName === 'react' || packageName === 'react-dom' || packageName.startsWith('react-');
    ...
    case 'rails':
      return packageName === 'rails' || packageName === 'actionpack' || 
             packageName === 'activerecord' || packageName === 'activestorage';
...
export function formatStackSummary(analysisResult: StackAwarePackageAnalysisResult): string {
  ...
  if (primaryStack) {
    ...
    if (primaryStack.parentStacks && Array.isArray(primaryStack.parentStacks) && primaryStack.parentStacks.length > 0) {
  ...
  const productionPackagesLength = Array.isArray(productionPackages) ? productionPackages.length : 0;
...
function getDisplayName(stackName: TechStackType): string {
  switch (stackName) {
    ...
    case 'rails': return 'Ruby on Rails';
