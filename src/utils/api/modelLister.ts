...
export function getAnthropicModels(): ModelInfo[] {
  const apiKeyStatus = getApiKeyForProvider('anthropic') ? 'available' : 'missing';
...
export function getOpenRouterModels(): ModelInfo[] {
  const apiKeyStatus = getApiKeyForProvider('openrouter') ? 'available' : 'missing';
 ...
 */
export function displayModels(models: ModelInfo[], showUnavailable: boolean = false): void {
  ...
  Object.entries(modelsByProvider).forEach(([provider, providerModels]) => {
    ...
    providerModels.forEach(model => {
      const status = model.apiKeyStatus === 'available'
        ? chalk.green('✓ Available')
        : chalk.red('✗ API Key Missing');
      
      logger.info(`  ${chalk.bold(model.displayName)} (${model.name})`);
      logger.info(`    ${model.description}`);
      logger.info(`    Context Window: ${model.contextWindow?.toLocaleString() || 'Unknown'} tokens`);
