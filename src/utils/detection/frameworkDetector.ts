 ...
 */
export async function detectFramework(projectPath: string): Promise<FrameworkDetectionResult | null> {
  try {
    ...
    if (FRAMEWORK_SIGNATURES[language]) {
      ...
      if (frameworkResult) {
        logger.debug(`Detected framework: ${frameworkResult.framework} with confidence ${frameworkResult.confidence.toFixed(2)}`);
        if (frameworkResult.additionalFrameworks && frameworkResult.additionalFrameworks.length > 0) {
          logger.debug(`Additional frameworks detected: ${frameworkResult.additionalFrameworks.join(', ')}`);
        }
    ...
    logger.error(`Error detecting framework: ${error instanceof Error ? error.message : String(error)}`);
    return null;
  }
}
...
export async function detectPrimaryLanguage(projectPath: string): Promise<string | null> {
  try {
    ...
    logger.error(`Error detecting primary language: ${error instanceof Error ? error.message : String(error)}`);
    return null;
  }
}
...
async function detectFrameworkForLanguage(
  ...
  try {
    ...
    logger.error(`Error detecting framework for ${language}: ${error instanceof Error ? error.message : String(error)}`);
    return null;
  }
}
...
async function getDependencies(
  ...
  try {
    ...
    logger.warn(`Error getting dependencies: ${error instanceof Error ? error.message : String(error)}`);
  }
...
async function detectCssFrameworks(
  ...
  try {
    ...
    logger.warn(`Error detecting CSS frameworks: ${error instanceof Error ? error.message : String(error)}`);
    return [];
  }
}